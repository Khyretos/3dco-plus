...
void writeJson(Model &m, const std::string &path) {
  ...
  for (size_t i = 0; i < m.meshes.size(); ++i) {
    ...
    json << "      \"use_custom_material\": "
         << (mesh.use_custom_material ? "true" : "false") << ",\n";
    json << "      \"use_custom_textures\": "
         << (mesh.use_custom_textures ? "true" : "false") << ",\n";
    ...
    // readInfoJson() below) - meaning any texture a user added was
    // silently lost the moment the model reloaded or the app
    // restarted, which looked exactly like "the texture doesn't
    // display"/UV mapping being broken, even though neither was
    // actually at fault. id isn't saved (a runtime GL handle,
    // regenerated by loadTexture() on read - see readInfoJson()).
    json << "      \"textures\": [";
    for (size_t ti = 0; ti < mesh.textures.size(); ++ti) {
      const Texture &t = mesh.textures[ti];
      json << "\n        {\n";
      ...
      json << "          \"wrapX\": " << t.wrapX << ",\n";
      json << "          \"wrapY\": " << t.wrapY << ",\n";
      json << "          \"offsetX\": " << t.offsetX << ",\n";
      json << "          \"offsetY\": " << t.offsetY << ",\n";
      json << "          \"scaleX\": " << t.scaleX << ",\n";
      json << "          \"scaleY\": " << t.scaleY << ",\n";
      json << "          \"rotation\": " << t.rotation << ",\n";
      json << "          \"flipX\": " << (t.flipX ? "true" : "false") << ",\n";
      json << "          \"flipY\": " << (t.flipY ? "true" : "false") << ",\n";
      json << "          \"border\": [" << t.border[0] << ", " << t.border[1]
           << ", " << t.border[2] << ", " << t.border[3] << "]\n";
      json << "        }" << (ti < mesh.textures.size() - 1 ? "," : "") << "\n";
    }
    json << "      ]\n";
    json << "    }" << (i < m.meshes.size() - 1 ? "," : "") << "\n";
  }
  json << "  ],\n";
  // Global textures - model-wide fallbacks (see Model::globalTextures's
  // own doc comment in model.h), saved once here rather than per-part
  // like the "textures" array above, since they aren't tied to any
  // one mesh. Same field set (and same array format) as a per-part
  // texture entry - id isn't saved for the same reason a per-part
  // texture's isn't (a runtime GL handle, regenerated by
  // loadTexture() on read).
  json << "  \"global_textures\": [";
  for (size_t ti = 0; ti < m.globalTextures.size(); ++ti) {
    const Texture &t = m.globalTextures[ti];
    json << "\n    {\n";
    json << "      \"path\": \"" << escapeJson(t.path) << "\",\n";
    json << "      \"name\": \"" << escapeJson(t.name) << "\",\n";
    json << "      \"type\": " << t.type << ",\n";
    ...
    json << "      \"border\": [" << t.border[0] << ", " << t.border[1] << ", "
         << t.border[2] << ", " << t.border[3] << "]\n";
    json << "    }" << (ti < m.globalTextures.size() - 1 ? "," : "") << "\n";
  }
  json << "  ],\n";
  json << "  \"global_material\": {\n";
  json << "    \"ambient\": " << m.globalMaterial.ambient << ",\n";
  json << "    \"diffuse\": " << m.globalMaterial.diffuse << ",\n";
  json << "    \"specular\": " << m.globalMaterial.specular << ",\n";
  json << "    \"shininess\": " << m.globalMaterial.shininess << ",\n";
  json << "    \"color\": [" << m.globalMaterial.color[0] << ", "
       << m.globalMaterial.color[1] << ", " << m.globalMaterial.color[2]
       << "],\n";
  json << "    \"alpha\": " << m.globalMaterial.alpha << "\n";
  json << "  },\n";
...
void readInfoJson(Model &m, const std::string &path) {
  ...
  for (auto &p : parts) {
    ...
    // Three possible sources for this, in priority order:
    //   - "use_custom_material" (current key, also the same key used by
    //     the very first version of this feature): true = use own values.
    //   - "use_global_material" (intermediate key from when the field
    //     was momentarily inverted): true = use global, so the value
    //     gets inverted back into use_custom_material here.
    //   - neither present (model saved before any of this existed,
    ...
    // loadTexture() regenerates it from the saved path here, the same
    // way it already does when a texture is first added via Settings.
    if (p.contains("textures") && p["textures"].is_array()) {
      for (auto &tex_json : p["textures"]) {
        std::string tex_path = tex_json.value("path", "");
        if (tex_path.empty())
          continue;
        ...
        Texture t;
        t.path = tex_path;
        t.name = tex_json.value("name", "");
        t.type = tex_json.value("type", 0);
        t.wrapX = tex_json.value("wrapX", 0);
        t.wrapY = tex_json.value("wrapY", 0);
        t.offsetX = tex_json.value("offsetX", 0.0f);
        t.offsetY = tex_json.value("offsetY", 0.0f);
        t.scaleX = tex_json.value("scaleX", 1.0f);
        t.scaleY = tex_json.value("scaleY", 1.0f);
        t.rotation = tex_json.value("rotation", 0.0f);
        t.flipX = tex_json.value("flipX", false);
        t.flipY = tex_json.value("flipY", false);
        if (tex_json.contains("border") && tex_json["border"].is_array() &&
            tex_json["border"].size() >= 4) {
          for (int bi = 0; bi < 4; ++bi)
            t.border[bi] = tex_json["border"][bi].get<float>();
        }
        loadTexture(t.id, t.path);
  ...
  // per-part texture (id regenerated via loadTexture(), never read
  // from JSON directly), skipped entirely (rather than left partially
  // set) if its image no longer exists on disk. Frees the previous
  // set's own GL handles first - same reasoning as loadModel()'s own
  // per-mesh version of this fix, since this function can run again
  // on an already-populated Model (switching models while a window
  // stays open) and would otherwise leak whatever was here before.
  for (Texture &tex : m.globalTextures)
    deleteTexture(tex.id);
  m.globalTextures.clear();
  auto loadOneGlobalTexture = [&](const json &gt) {
    std::string tex_path = gt.value("path", "");
    if (tex_path.empty())
      return;
    if (!std::filesystem::exists(tex_path)) {
    ...
    Texture t;
    t.path = tex_path;
    t.name = gt.value("name", "");
    t.type = gt.value("type", 0);
    t.wrapX = gt.value("wrapX", 0);
    t.wrapY = gt.value("wrapY", 0);
    t.offsetX = gt.value("offsetX", 0.0f);
    t.offsetY = gt.value("offsetY", 0.0f);
    t.scaleX = gt.value("scaleX", 1.0f);
    t.scaleY = gt.value("scaleY", 1.0f);
    t.rotation = gt.value("rotation", 0.0f);
    t.flipX = gt.value("flipX", false);
    t.flipY = gt.value("flipY", false);
    if (gt.contains("border") && gt["border"].is_array() &&
        gt["border"].size() >= 4) {
      for (int bi = 0; bi < 4; ++bi)
        t.border[bi] = gt["border"][bi].get<float>();
    }
    loadTexture(t.id, t.path);
    m.globalTextures.push_back(t);
  };
  if (data.contains("global_textures") && data["global_textures"].is_array()) {
    for (auto &gt : data["global_textures"])
      loadOneGlobalTexture(gt);
  } else if (data.contains("global_texture") &&
             data["global_texture"].is_object()) {
    // Migrates a model saved before global textures supported more
    // than one Diffuse entry - the old single-object "global_texture"
    // key becomes the first (and only) entry of the new array here,
    // and gets re-saved in the new format the next time this model is
...
void loadModel(Model &m, std::string path) {
  m.path = path;
  // Free every existing mesh's own GL texture handles before the
  // Mesh objects holding them are discarded - m.meshes.clear() below
  // (or the reassignment further down for the legacy info.txt path)
  // otherwise drops them with nothing left holding a reference to
  // pass to deleteTexture(), silently leaking one GPU texture per
  // entry every time a different model gets loaded into this same
  // Model (e.g. switching the model dropdown while a window stays
  // open) - pre-existing, not new to this pass, but the same leak
  // class as globalTextures's own version of this below.
  ...
  if (std::filesystem::exists(txtPath)) {
    ...
    // This format predates source/description/global material
    // entirely and never sets any of them - same reused-Model
    // reasoning as the JSON path's own fix just above, so a model
    // saved this way doesn't inherit whatever the previously-loaded
    // model's own values happened to be.
    m.source.clear();
    m.description.clear();
...
void loadMesh(Mesh &m, std::string path) {
  ...
  // Tracks the actual spread of UVs used by this mesh (after resolving
  // each vertex's texcoord index, not just the raw "vt" list) - a mesh
  // whose UVs are all missing/identical will have near-zero spread on
  // both axes, which is exactly what happens when a mesh optimization/
  // export tool drops or never generates a UV unwrap: every vertex
  // silently ends up sampling the same single point of any texture
  // applied to it, rendering as one flat color instead of the actual
  // image, which is easy to mistake for "texturing isn't working" or
  // "this app doesn't use UV coordinates" (it does - see loadTexture()/
  // CalcTexCoords() in shaders.cpp - this is purely a data problem with
  // the source mesh, not a mapping-method mismatch).
  ...
  for (size_t i = 0; i < vertices.size(); i++) {
    ...
    if (tex_idx >= 0 && tex_idx < (int)texcoords.size()) {
      ...
      uvMinU = std::min(uvMinU, texcoords[tex_idx].x);
      uvMaxU = std::max(uvMaxU, texcoords[tex_idx].x);
      uvMinV = std::min(uvMinV, texcoords[tex_idx].y);
      uvMaxV = std::max(uvMaxV, texcoords[tex_idx].y);
    } else {
      vertex_data.push_back(0.0f);
      vertex_data.push_back(0.0f);
      uvMinU = uvMaxU = uvMinV = uvMaxV = 0.0f;
    }
  }

  if (!vertices.empty() && (uvMaxU - uvMinU) < 0.0001f &&
      (uvMaxV - uvMinV) < 0.0001f) {
    spdlog::warn(
        "Mesh '{}' has no meaningful UV variation (every vertex maps to "
        "roughly the same texture coordinate, ({:.4f}, {:.4f})) - any "
        "texture applied to it will render as one flat color sampled "
        "from that single point, not the actual image. This is a "
        "property of the mesh file itself (missing or degenerate UV "
        "unwrap, e.g. after aggressive mesh optimization/decimation "
        "that didn't preserve or regenerate UVs), not a texture-mapping "
        "setting in the app - 3dco+ always uses the mesh's own UV "
        "coordinates (OBJ 'vt' lines, or the imported format's texture "
        "coordinate channel), never object-space/triplanar/normal-"
        "based mapping. Re-export this mesh with a proper UV unwrap to "
        "fix it.",
        path, uvMinU, uvMinV);
  }
...
void drawMesh(const Mesh &mesh, const glm::mat4 &modelMatrix, GLuint shader,
  ...
  // that hasn't opted out via use_custom_material - see that field's
  // doc comment in model.h for why this is all-or-nothing rather than
  // per-field. The model pointer is passed in as globalTextures
  // already is (see drawMesh's own declaration) - if it's null (the
  // standalone-draw path some callers use), fall back to the mesh's
  // own values so nothing about that path changes.
  const Material &mat = (globalMaterial && !mesh.use_custom_material)
                            ? globalMaterial->globalMaterial
                            : mesh.material;

  shaderUniformFloat(program, "material.ambient", mat.ambient);
  shaderUniformFloat(program, "material.diffuse", mat.diffuse);
  shaderUniformFloat(program, "material.specular", mat.specular);
  glm::vec4 matColor;
  if (baseColorOverride) {
    matColor = glm::vec4(*baseColorOverride, 1.0f);
  } else {
    matColor = glm::vec4(mat.color[0], mat.color[1], mat.color[2], 1.0f);
  }
  shaderUniformVec4(program, "material.color", matColor);
  shaderUniformFloat(program, "material.shininess", mat.shininess);
  shaderUniformFloat(program, "material.alpha", mat.alpha);
...
void importModelFile(Model &m, const std::string &filepath) {
  ...
  if (!scene || !scene->mRootNode) {
    ...
    spdlog::error("Failed to import model: {}", assimpError);

    // .blend files hit a genuinely long-standing, recurring class of
    // bug in Assimp's own hand-written Blender parser - it maintains
    // its own reader for Blender's internal DNA/SDNA binary layout,
    // which has to be kept in sync with every Blender version's
    // internal structural changes, and it's fallen behind repeatedly
    // over the last decade+ (the exact same "Number of X is larger
    // than the corresponding array" family of error has been reported
    // against UV-unwrapped Blender meshes since at least 2014 -
    // assimp/assimp#325 - and still reproduces against current
    // Blender 4.x saves and the current Assimp release). This isn't
    // something a quick fix here can paper over, so rather than leave
    // a cryptic Assimp error as the only thing the user sees, point
    // at the actually-reliable path: Blender's own OBJ/FBX/glTF
    // exporters, which this app's import (and the rest of its model
    // pipeline) handles correctly and are what the built-in library's
    // own models use.
    ...
    if (ext == ".blend") {
      spdlog::error(
          "Direct .blend import is unreliable for many Blender files "
          "(this is a long-standing limitation in the Assimp library's "
          "own Blender parser, not something specific to this file) - "
          "try exporting from Blender as OBJ, FBX, or glTF instead "
          "(File > Export in Blender) and importing that instead. "
          "Assimp's error was: {}",
          assimpError);
    }
    return;
