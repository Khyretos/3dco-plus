...
// lot by platform:
//
// - Windows: fully implemented here (Shell_NotifyIcon + a native popup
//   menu). This is the only platform with a real implementation right
//   now - isSupported() returns true only here.
// - macOS: needs an NSStatusItem, which is Objective-C API - not
//   reachable from plain C++ without an .mm (Objective-C++) file. Not
//   implemented yet; isSupported() returns false, and every call here
//   is a safe no-op.
...
//   Shell still doesn't without a user-installed extension. Not
//   implemented yet either; isSupported() returns false there too.
//
...
// True only on platforms with a real implementation (Windows, for now).
// The settings UI should hide its tray-icon toggle entirely when this
// is false, rather than showing a control that can't do anything.
bool isSupported();
